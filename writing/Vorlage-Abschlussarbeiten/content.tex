...
the orbit classes between members of the same horizental gangs, and that 
leads to a new set of orbit classes that for all their assignements there exists a weak 
Hanani-Tutte drawing in $G^*$.
...
\begin{lemma}
    let $\left( w_1, \ldots, w_j \right)$ subset $HG_{A}$of vertices of a horizental gang $HG$
    with no assignement of the orbit classes in $HG_{A}$ resulting in a cylic relation. 
    let $w_{j+1} \in HG$
    After the algorithm~\ref{alg:MAEEq} treats $w_{j+1}$ in the loop. $HG_{A} \cup \{w_{j+1}\}$ has no
    assignement of the orbit classes in $HG_{A} \cup \{w_{j+1}\}$ that lead to a cyclic relation between 
    some of the members of the subset $HG_{A} \cup \{w_{j+1}\}$
\end{lemma}
\begin{proof}
    ...
    To prove this we will prove by induction that merging the equivalence class between $w_{j+1}$ and the members of $HG_{i+1}^j$ hold the two
    invariants. 
    Let $\mathrm{VHG}$ the set of visited members of the horizental gank $HG_{i+1}^j$.

    For the base case we have only one visited member $\mathrm{VHG}_1 = \left\{w_1\right\}$ from the gang $HG_{i+1}^j$.
    Which means we are considering only one equivalence class $eq\left( w_1, w_{j+1} \right)$, which doesn't lead to any cyclic 
    relation since we need a three vertices at least for that to be possible.

    For the induction step, let $\mathrm{VHG}_k = \left( w_1, \ldots, w_k \right) $  with $k < j$. The equivalence class 
    let's consider the $eq\left( w_k, w_{j+1} \right)$. If the equivalence class origin is different than the root equivalence class
    origin then we merge $eq\left( w_k, w_{j+1} \right)$  with the root equivalence class $eq\left( w_0, w_1 \right)$. 

    This first keeps the first invariance (TODO give a name to the invariance?) because merging $eq\left( w_k, w_{j+1} \right)$  and 
    ...
    since we are just merging equivalence classes and not adding any new relations between vertices the planarity constraint is also respected.
    For the second invariant (TODO give it a name), let's consider prove by contradiction that merging the origin equivalence class of $w_k$ and $w_{j+1}$ ,with 
    ...
    that we didn't find a member of the gang that must be after $w_{j+1}$. `That can't happen if the base of equivalence class  $\left( w_k, w_{j+1} \right)$ 
    is not part of the base of the parent equivalence class, because whatever the direction of the base of the equivalence class $\left( w_k, w_{j+1} \right)$, 
    it doesn't lead to a cyclic relation if we merge the origin of that equivalence with one of the direction with the parent equivalent class.
    For the case where we only have the parent equivalence class between the triple. It can't be an original equivalence class as these don't lead 
    to a cyclic relation (TODO lemma?). It the the equivalence class in the triple is a result of one or multiple merges, We know that a triple of
    gang members that can have a cyclic relation don't share more than one edge with any other triple of gang members (lemma) which means that, we can't 
    have this case since if we had at least one merge of the equivalence class in the triple it means at least two equivalence classes where part 
    ...
    with $x \in \mathrm{VHG}_k - \left\{w_k\right\}$, they can induce two equivalence classes or one. In the case of two, the base equivalence class between $x, w_{j+1}$ is 
    of the direction from $w_{j+1}$ to $x$ and the base equivalence class $x, w_k$ is of a direction from $x$ to $w_k$ since $x$ comes before $w_k$ in visited linked
    list. Since we are in the reverse mode of the algorithm, the algorithm will merge the base equivalence class of $w_k, w_{j+1}$ with the direction from 
    $w_{j+1}$ to $w_k$ with the other mentionned induced equivalence classes, which contradicts. For the case, where we have only one equivalence class induced from the triple, it's 
    not possible due to the same contradiction from the first mode.

    The merge of the appropriate equivalence class from the base of $\left( w_{j+1}, w_k \right)$ doesn't affect any existing triple order, since we are just 
    changing the new equivalence class. 

    Therefore merging the equivalence classes between $w_{j+1}$ and the members of $HG_{i+1}^j$ ,when needed, hold the two invariants. 

    Thus adding the vertex member $w_{j+1}$ to the gang $HG_{i+1}^j$ will hold both invariants. 
...
gang memberes of $HG_{i+1}$ from the set of resulted equivalence classes. 

Therefore, we proved that the algorithm removes all the cyclic relations between any members of the same horizental gang. Which is all what we need to prove 
since the proof of transforming strong hanani-tutte drawing to weak hanai-tutte drawing from theorem 3.1 from .. induce on the the number of odd pairs between adjacent edges.
The gang members are only the vertices that can lead to the odd crossings described in theorem 3.1.  
\end{proof}
