...
\label{sub:Algorithm description}
    ...
    For the first task, we consider vertices in $V_{\mathrm{v}}^{\left( h - 1 \right)}$ adjacent to the vertex $v$ on the level $h-1$. We compute an ordering $\mathcal{V}$ of the vertices in $V_{\mathrm{v}}^{\left( h - 1 \right)} $
